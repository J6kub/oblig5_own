...
def init_db(print_progress=False):
    ...
    try:
        ...
        tbl_foresatt = TableCsv("DB_foresatt.csv",splitter,progress=print_progress)
    #init barnehage DB
    try:
        ...
        tbl_barnehager = TableCsv("DB_barnehager.csv",splitter,progress=print_progress)
    #init barn DB
    try:
        tbl_barn = CreateCsvTable("DB_barn.csv", ["id", "navn", "pnr", "foresatt_id","barnehage_id"],progress=print_progress)
    except ValueError:
        tbl_barn = TableCsv("DB_barn.csv",splitter,progress=print_progress)
    #init barn DB
    try:
        ...
        tbl_soknad = TableCsv("DB_soknad.csv",splitter,progress=print_progress)
init_db(True)
